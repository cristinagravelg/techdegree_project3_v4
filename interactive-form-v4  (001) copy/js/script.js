...
form.addEventListener("submit", (event)=>{
    ...
    if (isCreditCardValid(cardInput.value)==false){
        showErrorMessage("card");
        event.preventDefault();
    }
    if (isCreditZipCodeValid(zipcodeInput.value)==false){
        showErrorMessage("zipcode");
        event.preventDefault();
    }
    if (isCVVValid(cvvInput.value)==false){
        showErrorMessage("cvv");
        event.preventDefault();
    }
    if (selectedCheckboxes == 0){
        showErrorMessage("activities");
        event.preventDefault();
    }
    else{
        submitError.style.display ="none";
